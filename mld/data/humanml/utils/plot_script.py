...
def simple_plot_3d_motion(
    ...
    """
    Very simple 3D stick-figure animation for HumanML3D-style joints.
    joints: (T, J, 3) in world coordinates.
    kinematic_tree: list of chains, e.g. [[0,2,5,8,11], ...]
    """
    ...
    data = np.asarray(joints)
    assert data.ndim == 3, f"Expected (T,J,3), got {data.shape}"
    T, J, C = data.shape
    assert C == 3

    # Compute bounds from data so we *always* see something
    ...
    # Create line objects for each chain
    lines = []
    ...
    def update(frame_idx):
        ...
        x = frame[:, 0] - root[0]
        z = frame[:, 2] - root[2]
        y = frame[:, 1] - mins[1]  # put feet near ground

        for line, chain in zip(lines, kinematic_tree):
            pts = np.stack([x[chain], z[chain], y[chain]], axis=-1)
            line.set_data(pts[:, 0], pts[:, 1])
            line.set_3d_properties(pts[:, 2])

        return lines

    ani = FuncAnimation(fig, update, frames=T, interval=1000 / fps, blit=False, repeat=False)

    # Let matplotlib pick a writer; if ffmpeg is installed, it'll use it.
    ani.save(save_path, fps=fps)
    plt.close(fig)
