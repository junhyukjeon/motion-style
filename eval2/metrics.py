...
class TM2TMetrics(Metric):
    ...
    def compute(self, sanity_flag=False):
        ...
        all_texts = torch.cat(self.text_embeddings, dim=0).cpu()[shuffle_idx, :]
        all_genmotions = torch.cat(self.recmotion_embeddings, dim=0).cpu()[shuffle_idx, :]
        all_gtmotions = torch.cat(self.gtmotion_embeddings, dim=0).cpu()[shuffle_idx, :]

    
        all_joints_rst = self.joints_rst

        all_predicted = torch.cat(self.predicted, dim=0).cpu()[shuffle_idx, :]
        
        all_label = torch.cat(self.label, dim=0).cpu()[shuffle_idx]
        ...
        for index in range(0,len(all_joints_rst)):
            ...
            skate_ratio_sum += skate_ratio.mean()
        ...
        metrics["Inference_time"] = all_time / len(all_time_list)
