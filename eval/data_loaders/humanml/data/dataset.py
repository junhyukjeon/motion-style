...
class Text2MotionDatasetV2(data.Dataset):
    def __init__(self, opt, mean, std, split_file, w_vectorizer):
        ...
        cache_path = os.path.join(opt.meta_dir, self.opt.dataset_name + '_' + _split + '.npy')
...
class HumanML3D(data.Dataset):
    def __init__(self, mode, datapath='./dataset/humanml_opt.txt', split="train", **kwargs):
        ...
        if not os.path.exists(opt.data_root):
            opt.data_root = opt.data_root.replace("HumanML3D", "humanml3d")
            opt.motion_dir = opt.motion_dir.replace("HumanML3D", "humanml3d")
            opt.text_dir = opt.text_dir.replace("HumanML3D", "humanml3d")
