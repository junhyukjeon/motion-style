...
def plot_tsne(model, loader, device, epoch=None, title="valid", result_dir="", label_to_name_dict=None, max_samples=3000, writer=None):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            all_embeddings.append(style.cpu())
            all_labels.append(style_idx.cpu())
...
def plot_tsne_mix(model, loader, device, epoch=None, title="valid", result_dir="", label_to_name_dict=None, max_samples=3000, writer=None):
    model.eval()
    all_embeddings, all_labels = [], []
